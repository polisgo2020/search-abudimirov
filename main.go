...
func makeIndexFile(dirPath string, files []os.FileInfo) (map[string]string, error) {
	...
	wordLoop:
		for j := range str {
			...
			//эту часть принципиально не знал как реализовать. взято из репы другого студента. работает верно, но не могу понять что и зачем делает этот код "strconv.Itoa(int(value[len(value)-1])-48)" :( ????
